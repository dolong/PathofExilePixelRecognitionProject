	  ...
	  If $rare  > 0 Then
		 ...
		 if $scroll > 0 Then	
			   Local $pos = MouseGetPos()
				  ;MouseMove($xS, $yS)
				  MouseClick("secondary", $xS, $yS, 3, 0)
				  sleep(200)
...
if	$portalTowns < 6 then
   If Not checkIsInTown() and  Then	  
	  writeLog("Portal Error, Resetting Portal." & checkIsInTown() & " = in Town")
	  MouseClick("primary",648, 473,3, 0) 
	  Sleep(1000)
	  Send("{q}")  
	  Sleep(5500)
	  ...
	  MouseClick("primary", $PixelLoc[0], $PixelLoc[1], 3, 0)   
		 ...
		 $PixelLoc = PixelSearch(0, 150, @DesktopWidth, @DesktopHeight, 0xe1e26a) ;rares
		   If IsArray($PixelLoc) = True Then
			 MouseClick("primary", $PixelLoc[0], $PixelLoc[1], 3, 0)
		  Else
			 $PixelLoc = PixelSearch(0, 150, @DesktopWidth, @DesktopHeight, 0x18948e) ;gem
			If IsArray($PixelLoc) = True Then			   
			MouseClick("primary", $PixelLoc[0], $PixelLoc[1], 3, 0)
			Else			
			 $PixelLoc = PixelSearch(0, 150, @DesktopWidth, @DesktopHeight, 0x87979e3) ;magic
			   If IsArray($PixelLoc) = True Then			   
			   MouseClick("primary", $PixelLoc[0], $PixelLoc[1], 3, 0)
			   EndIf
			EndIf
